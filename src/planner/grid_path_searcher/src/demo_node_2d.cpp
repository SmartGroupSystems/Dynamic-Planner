...
//定义了结构体AstarPathFinder变量_astar_path_finder
//该结构体存储、实现了Astar路径规划所需的所有信息和功能
AstarPathFinder2d * _astar_path_finder     = new AstarPathFinder2d();
JPSPathFinder   * _jps_path_finder       = new JPSPathFinder();

void rcvWaypointsCallback(const nav_msgs::Path & wp);                                                     //订阅到终点信息的cb函数
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 & pointcloud_map);//订阅到地图信息的cb函数

void visGridPath( vector<Vector2d> nodes, bool is_use_jps );                                            //可视化函数
void visVisitedNode( vector<Vector2d> nodes );                                            
void pathFinding(const Vector2d start_pt, const Vector2d target_pt);                           //路径规划函数
...
{   
    ...
    //FIXME 需要修改has_map以适应多次调用
    if(_has_map ) return;//验证是否已有地图
